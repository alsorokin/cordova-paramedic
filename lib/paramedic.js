...
ParamedicRunner.prototype.runLocalTests = function () {
    ...
    // checking for Windows Store platform here because in this case we still need to start the app to know its id
    // will check again and skip tests a bit lower
    if (!self.config.runMainTests() && !self.config.isWindowsStore()) {
    ...
    .then(function (output) {
        // if running on Windows Storem we need to parse output 
        // for appId which we'll need to run Appium tests
        if (self.config.isWindowsStore() && self.config.runAppiumTests()) {
            self.targetObj = {
                'appId': /^ActivateApplication:\s+(.*)$/gm.exec(output)[1],
                'target': 'WindowsPC'
            }
        }
        if (!self.config.runMainTests()) {
...
ParamedicRunner.prototype.runAppiumTests = function (useSauce) {
    ...
    if (platform !== 'android' && platform !== 'ios' && !self.config.isWindowsStore()) {
    ...
    if (platform === 'windows') {
        options.app = self.targetObj.appId;
    }
    if (useSauce) {
...
ParamedicRunner.prototype.getCommandForStartingTests = function () {
    ...
    if(self.config.getAction() === 'build' || self.config.isWindowsStore()) {
